  ...
  install: function (Vue, options) {
    ...
    Vue.directive('rightMenu', {
      ...
      bind(el, binding, vnode) {
        ...
        // var Maskstyle = "position:fixed;top:0;left:0;width:100%;height:100%;z-index:100;"
        ...
        el.addEventListener('contextmenu', (e) => {
          // console.log(sessionStorage.getItem('docMenu'))
          if (sessionStorage.getItem('docMenu') == 'true' && binding.value['dit'] == 'docRightMenu') {
            return null
          }
          ...
          document.getElementById('TT_MASK').style.position = 'fixed'
          document.getElementById('TT_MASK').style.top = '0'
          document.getElementById('TT_MASK').style.left = '0'
          document.getElementById('TT_MASK').style.width = '100%'
          document.getElementById('TT_MASK').style.height = '100%'
          document.getElementById('TT_MASK').style.zIndex = '100'
          document.getElementById('TT_MASK').style.display = 'block'
          ...
          if (!document.getElementById('tt_right_menu' + currentSize)) {
            ...
            binding.value['menus'].map((item) => {
              ...
              optionp.classList.add('menuRightItem')
              // 控制icon位置
              let iconPosition = ''
              // icon内容
              let icon = ''
              ...
              optionp.style.textAlign = 'center'
              optionp.style.overflow = 'hidden'
              optionp.style.textOverflow = 'ellipsis'
              optionp.style.whiteSpace = 'nowrap'
              optionp.style.userSelect = 'none'
              optionp.style.msUserSelect = 'none'
              optionp.style.webkitUserSelect = 'none'
              optionp.style.color = 'rgb(102, 102, 102)'
              optionp.style.fontSize = '12px'
              optionp.style.padding = '0px 5px'
              optionp.style.borderBottom = '1px solid rgb(255, 255, 255)'
              optionp.style.background = 'rgb(255, 255, 255)'
              optionp.style.lineHeight = '30px'
            ...
            if (binding.value['dit'] == 'docInforManage') {
              boxDiv.setAttribute('class', 'docInforManageMenu')
            }
            // 菜单的样式
            // boxDiv.style = `background:#fff;color:#333;${binding.value["boxStyle"] ? binding.value["boxStyle"] : ""};position:fixed;z-index:200;top:${menuY}px;left:${menuX}px;`
            boxDiv.style.color = 'rgb(51, 51, 51)'
            boxDiv.style.width = '120px'
            boxDiv.style.background = 'rgb(255, 255, 255)'
            boxDiv.style.boxShadow = 'rgb(172, 172, 172) 0px 0px 15px'
            boxDiv.style.padding = '10px 0px'
            boxDiv.style.position = 'fixed'
            boxDiv.style.zIndex = '200'
            boxDiv.style.top = menuY + 'px'
            boxDiv.style.left = menuX + 'px'
            ...
            // console.log(boxDiv)
            // boxDiv.style = `color:#333;background:#fff;${binding.value["boxStyle"] ? binding.value["boxStyle"] : ""};position:fixed;z-index:200;top:${menuY}px;left:${menuX}px;`
            boxDiv.style.color = 'rgb(51, 51, 51)'
            boxDiv.style.width = '120px'
            boxDiv.style.background = 'rgb(255, 255, 255)'
            boxDiv.style.boxShadow = 'rgb(172, 172, 172) 0px 0px 15px'
