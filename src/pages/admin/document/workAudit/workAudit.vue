          ...
          <el-main>
            <el-tabs>
              <div class="switchHeight_box">
                <div style="font-size: 28px;font-weight: bold;display:flex;align-items: center;">单位文档审核</div>
                <div style="display:flex;align-items: center;">
                  <i class="el-icon-s-order"
                     style="color: #ccc;font-size: 18px; cursor:pointer;"></i><span @click="uploadRecord"
                        style="cursor:pointer;margin-left:10px">审核记录</span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    examine (index, row) {
      let that = this
      let title = '文档审核'
      that.$bus.$emit('closeTabsItem', 'workAudit')
      let routerPath = '/documentBrowse'
      let tabObj = { title, routerPath }
      that.setTabsList(tabObj)
      that.$router.push({ path: routerPath, query: { row: row, folderType: '3' } })
    },
    ...
    async getDocList (params) {
      this.loading = true
      this.$get('/epf-document/docdocumentinfo/getMyExamineWorkDocs', params).then(res => {
        console.log('871===========', res)
        if (res.code != '0') {
