            ...
            <el-tabs>
              <div class="switchHeight_box">
                <el-row>
                  <el-col :span="10">
                    <el-col>
                      <span style="min-height:40px;font-size: 28px;line-height:40px; font-weight: bold;">单位文档</span>
                      <p class="tabs"
              ...
              <div class="main-table"
                ...
                <!-- 切换后的排列模式 -->
                <el-main :style="{'height':menuListHeight-150 + 'px','borderLeft':'1px solid rgb(224, 224, 224)','borderBottom':'1px solid rgb(224, 224, 224)','borderRight':'1px solid rgb(224, 224, 224)'}"
                         v-if="!tablelist">
                  <el-row>
                  </el-row>
                    ...
                    <el-checkbox-group v-model="checkedCities"
                      ...
                      <el-col class="carditem"
                        ...
                        <div class="docImgBox"
                          ...
                          <img width="70px"
                               height="70px"
                               v-if="'png,gif,jpeg,jpg'.indexOf(item.extName.toLowerCase()) != -1  "
          ...
          <div class="knowledgeRight">
            <el-aside width="300px"
              ...
              <!-- <el-header style="height:40px;line-height:40px;display:flex;justify-content: space-between;align-items: center;"
                       class="m_title"><span style="font-size: 18px;
    padding-left: 12px;">常用标签</span><span @click="AddFlowpatreason"
                    style="cursor: pointer;color:rgba(80,165,244,1);">编辑</span> </el-header> -->
      ...
      <el-dialog title="收藏"
        ...
        <div style="
          ...
          ">
          <div class="choice">
            <p style="padding-bottom: 10px;font-size:16px;">选择收藏夹 <span class="el-icon-folder-add"
                    style="font-size:16px;"
                    @click="addCollect">新建收藏夹</span></p>
...
export default {
  ...
  methods: {
    ...
    tosearch () {
      ...
      erd.listenTo(document.getElementById("workFiles"), element => {
        _this.$nextTick(() => {
          setTimeout(() => {
            //监听到事件后执行的业务逻辑
            this.treeHeight = this.$refs.dwBOX.offsetHeight - 276;
          }, 500);
        });
      });
    },
    ...
    loadTree () {
      var that = this;
      this.loading2 = true;
      let params = { folderType: that.folderType, showDocCount: "YES" };
      that.$get("/epf-document/admin/docfolder/getFolderByType", params).then(res => {
        ...
        //   res.ztreeJson[0].folderName = res.ztreeJson[0].folderName + '(' + res.ztreeJson[0].docCount +')'
        //   if(res.ztreeJson[0].childForder !== null) {
        //     for(var key2 in res.ztreeJson[0].childForder) {
        //       res.ztreeJson[0].childForder[key2].folderName = res.ztreeJson[0].childForder[key2].folderName + '(' + res.ztreeJson[0].childForder[key2].docCount +')'
        //       if(res.ztreeJson[0].childForder[key2].childForder !== null) {
        //         for(var key3 in res.ztreeJson[0].childForder[key2].childForder) {
        //           res.ztreeJson[0].childForder[key2].childForder[key3].folderName = res.ztreeJson[0].childForder[key2].childForder[key3].folderName + '(' + res.ztreeJson[0].childForder[key2].childForder[key3].docCount +')'
        //         }
        //       }
        //     }
        //   }
    ...
    uploadWrokFile () {
      let that = this;
      let title = '单位文档上传'
      let routerPath = `/libraryUpload?folderType=${that.folderType}&uplaodtype=1`
      let tabObj = { title, routerPath }
      that.setTabsList(tabObj)
  ...
  mounted () {
    ...
    this.$bus.$on('toLength2', val => {
      this.totlecorde = val
    })
  },
  beforeDestroy () {
    this.$bus.$off('toLength2');
  },
  filters: {
    numFilterScan (value) {
      let realVal = ''
