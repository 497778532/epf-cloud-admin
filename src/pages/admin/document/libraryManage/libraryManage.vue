            ...
            <div class="m_title">
              <div style="width:100%;height:30px;display:flex;justify-content: space-between;align-items: center;">
                ...
                <div style="color:#727272;"
                  ...
                  <div @click="initPwd"
                       style="margin-left:16px;"><img src="../images/导出.png"
                         alt=""><span>移动</span></div>
                  <!-- <div @click="initPwd"><i class="el-icon-s-promotion"></i>移动</div> -->
                  <!-- <div @click="showsingleuser"> <i class="el-icon-delete-solid"></i>回收站 </div> -->
                  <div @click="showsingleuser"
                       style="margin-left:16px;"><img src="../images/删除.png"
                         alt=""><span>回收站</span></div>
                  <!-- <span  class="button_bg" @click="editFlowpatreason" style="display: none;">编辑</span> -->
                  <!-- <div @click="exportDocument"><i class="el-icon-s-unfold"></i>导出exel</div> -->
                  <div @click="exportDocument"
                       style="margin-left:16px;"><img src="../images/下载.png"
                         alt=""><span>导出exel</span></div>
                </div>
              </div>
              <div class="tabs">
...
export default {
  ...
  mounted () {
    ...
    erd.listenTo(document.getElementById("libraryManage"), element => {
      _this.$nextTick(() => {
        setTimeout(() => {
          //监听到事件后执行的业务逻辑
          this.treeHeight = this.$refs.libBOX.offsetHeight + 50;
        }, 500);
      });
    });
  },
  methods: {
    ...
    upload () {
      let that = this;
      let title = '上传文档'
      let routerPath = '/libraryUpload?folderType=1'
      let tabObj = { title, routerPath }
      that.setTabsList(tabObj)
      that.$router.push({ path: routerPath, query: { uplaodtype: "0" } })
    },
    //新增文档
    addDocument () {
      let that = this;
      let title = '新增文档'
      let routerPath = '/articleManage'
      let tabObj = { title, routerPath }
      that.setTabsList(tabObj)
      that.$router.push({ path: routerPath, query: { editor: "" } })
    },
    async getDocList (params) {
      if (this.publishdata.length > 1) {
        params["starttime"] = this.publishdata[0];
        params["endtime"] = this.publishdata[1];
      } else {
        params["starttime"] = '';
        params["endtime"] = '';
      }
      ...
      this.$get("/epf-document/docdocumentinfo/getTenantDocJson", params).then(res => {
        this.toLength1 = res.pager.totalRecord
        if (res.code != 0) {
    ...
    exportDocument () {
      ...
      let ursStr = encodeURI("/api/epf-document/docdocumentinfo/exportExceldoc?token=" + token + "&" + str);
      window.open(ursStr);
    },
        ...
        .then(() => {
          that.$post("/epf-document/docdocumentinfo/movetodelet", { id: ids })
            .then(res => {
              ...
              that.$bus.$emit('toLenght1', that.totalRecord)
            });
        })
        .catch(() => { });
    },
  ...
  watch: {
    toLength1 (val) {
      this.$bus.$emit('toLength1', this.toLength1)
    },
    pageNo (val) {
