<template>
  <el-container id="landgradient"
                class="EPF-table">
    <el-main id="main">
      <epf-mapview name="landgradient22"
                   :delay="1">
        <div class="toolbar">
...
export default {
  ...
  methods: {
    myTab(key) {
      switch (key) {
        case 'city':
          this.getData({ evaluationType: 'evaluation_type-001' })
    ...
    myAppraisals1(ids1, ids2, key) {
      this.$get(
        ...
        {
          evaluationType: 'evaluation_type-001',
          analysisDataId: ids1,
          analysisDataId: ids2
        }
      ).then(res => {
  ...
  components: { ZoomItem, PanItem, JumptoItem, PlanSide, toolbar },
  created() {
    this.getData({ evaluationType: 'evaluation_type-001' })
