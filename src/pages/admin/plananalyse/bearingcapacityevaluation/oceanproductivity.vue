<template>
  <el-container id="main-container"
                class="EPF-table">
    <el-main id="main">
      <epf-mapview name="oceanproductivity"
                   id="projectOneMap">
        <div class="toolbar">
...
export default {
  name: 'oceanproductivity',
  mixins: [miXin],
  data() {
    return {
      childrenData: {
        title: '海洋资源初级生产力评价',
        type: 'oceanproductivity',
        name: '生产力评价'
      },
      analysisData: {
        showEnd: false
  ...
  methods: {
    myAppraisals1(ids1, ids2) {
      this.$get(
        ...
        {
          evaluationType: 'evaluation_type-007',
          analysisDataId: ids1,
          analysisDataId: ids2
        }
      ).then(res => {
  ...
  created() {
    this.getData({ evaluationType: 'evaluation_type-007' })
