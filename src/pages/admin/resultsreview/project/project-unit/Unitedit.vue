  ...
  data () {
    return {
      ...
      elFormRules: {
        ...
        planningLevel: [{ required: true, message: '必填项', trigger: 'change' }]
      }
    }
  },
  methods: {
    ...
    submitForm (val) {
      this.$refs[val].validate(async valid => {
        if (valid) {
          ...
          for (let i = 0; i < this.uploadSuccessArr.length; i++) {
            const obj = this.uploadSuccessArr[i]
            if (!obj.hasOwnProperty('deleteFlag')) {
              delete this.uploadSuccessArr[i]['id']
            }
          }
          ...
          let res = await axios({
            url: '/api/epf-result-review/preparatUnit/update',
            method: 'POST',
            data: params,
            headers: {
          ...
          if (res.code == 0) {
            this.$message.success('编制单位编辑成功') // 成功
            this.cancelClick()
          } else {
    ...
    async getFiles (id) {
      ...
      if (res.code === 0) {
        ...
        for (let i = 0; i < resultArr.length; i++) {
          ...
          res.result[i].deleteFlag = 0
        }
        this.uploadSuccessArr = res.result
        console.log(this.uploadSuccessArr)
      }
    },
    getInputData () {
