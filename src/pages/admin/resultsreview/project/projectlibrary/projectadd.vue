      ...
      <div class="filterWrap">
                ...
                <el-col :span="12">
                  <el-form-item label="编制单位名称："
                    ...
                    <el-autocomplete v-model="
                        projectForm.revProjectUndertakerParamList[index]
                          .companyName
                      "
              ...
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="各专业中级以上人数：">
                    <el-select v-model="
                        projectForm.revProjectUndertakerParamList[index]
                          .revProjectUndertakerNumberVoList[0].zhname
                      "
                               placeholder="请选择专业"
                               style="width:50%"
                               @change="
                        zhuanyeTypeSelect(
                          dictionary.professional_list,
                          projectForm.revProjectUndertakerParamList[index]
                            .revProjectUndertakerNumberVoList[0].zhname,
                          $event,
                          index,
                    ...
                    <el-input v-model="
                        projectForm.revProjectUndertakerParamList[index]
                          .revProjectUndertakerNumberVoList[0].peopleNumber
                      "
                              style="width:30%;"
                              @input="getCDInputData"
                              :maxlength="10">
                    ...
                    <el-form-item label="各专业中级以上人数：">
                      <el-select v-model="t.zhname"
                        ...
                        <el-option v-for="(item,
                          indexZD) in dictionary.professional_list"
                                   :ref="item.zhCn"
                                   :id="item.id"
                                   :dictKey="item.dictKey"
                                   :label="item.zhCn"
            ...
            <div class="uploadContentBox"
              ...
              <div class="fileNameBox"
                ...
                <i class="el-icon-delete"
                   @click="handleClickDelCorrelationFile(item)"></i>
              </div>
            </div>
          </div>
        ...
        <div class="thirdStep"
          ...
          <div class="submitTime">
            <span style="color:#1e7ae5;font-size:14px;">{{ count }}</span>
            秒后自动返回项目库
          </div>
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      projectForm: {
        ...
        administrativeCode: '440000', // 行政区代码
        resultType: '', // 成果类型
  ...
  methods: {
    ...
    async cancelForm () {
      if (this.projectForm.id !== '') {
        let res = await this.$post(
          `/epf-result-review/admin/revproject/delete?id=${this.projectForm.id}`
        )
        if (res.code !== 0) {
    ...
    saveForm (val) {
      this.$refs[val].validate(async valid => {
        if (valid) {
          // 是否投标
          if (
            this.projectForm.biddingFlag === 0
            ...
            for (var key in this.projectForm.revProjectUndertakerParamList) {
              if (
                this.projectForm.revProjectUndertakerParamList[key]
                  .preparatUnitId === ''
              ) {
                let index = key
                document.querySelector(
                  `#unitFormStyle${index} .el-autocomplete`
              ...
              if (
                document.querySelector(
                  `#unitFormStyle${index0} .el-autocomplete`
                ).style.borderRadius === '4px'
              ) {
                this.isCheck = false
              } else {
          ...
          // let whoHarf=''
          //           if(this.projectForm.id===''){
          // whoHarf='/api/epf-result-review/admin/revproject/save'
          //           }else{
          // whoHarf='/api/epf-result-review/admin/revproject/update'
          //           }

          let res = await axios({
            url: '/api/epf-result-review/admin/revproject/save',
            method: 'POST',
      ...
      this.$refs[val].validate(async valid => {
        if (valid) {
          ...
          if (this.projectForm.id === '') {
            ...
            projectFormIDMsg['projectTypeName'] = this.projectForm.projectTypeName
            projectFormIDMsg['planningProvince'] = this.projectForm.planningProvince
            projectFormIDMsg['planningCity'] = this.projectForm.planningCity
            projectFormIDMsg['planningArea'] = this.projectForm.planningArea
            projectFormIDMsg['administrativeCode'] = this.projectForm.administrativeCode
            let res = await axios({
              url: '/api/epf-result-review/admin/revproject/save',
              method: 'POST',
    ...
    // 删除附件
    handleClickDelCorrelationFile (item) {
      console.log(item)
      this.$confirm('此操作将永久删除该项目信息相关附件, 是否继续?', '提示', {
        ...
        .then(() => {
          if (item.hasOwnProperty('projectId') === false) {
            ...
            uploadUpdateList['revProjectFileEntityList'][0].documentId
              = item.documentId
            uploadUpdateList['revProjectFileEntityList'][0].documentName
              = item.documentName
            uploadUpdateList['revProjectFileEntityList'][0].extensionName
              = item.extensionName
            uploadUpdateList['revProjectFileEntityList'][0].fileType
              = item.fileType
            uploadUpdateList['revProjectFileEntityList'][0].fileSize
              = item.fileSize
    ...
    async getProjectType () {
      let res = await this.$get(
        `/epf-result-review/revplanningcategorymanage/getRevPlanningType?cantonId=${this.projectForm.administrativeCode}&orgId=${sessionStorage.getItem('orgId')}`
      )
      if (res.code === 0) {
    ...
    zhuanyeTypeSelect (dic, name, event, index, dex, number) {
      console.log(number)
      this.selectIdsArr = []
      for (var key in this.projectForm.revProjectUndertakerParamList[index]
        .revProjectUndertakerNumberVoList) {
        if (
          this.projectForm.revProjectUndertakerParamList[index]
            .revProjectUndertakerNumberVoList[key].zhname === '城乡规划相关专业'
        ) {
          let setIndex = key
          this.projectForm.revProjectUndertakerParamList[
            index
          ].revProjectUndertakerNumberVoList[setIndex].name
            = 'professional_list-002'
        } else if (
          this.projectForm.revProjectUndertakerParamList[index]
            .revProjectUndertakerNumberVoList[key].zhname === '土地管理相关专业'
        ) {
          let setIndex = key
          this.projectForm.revProjectUndertakerParamList[
            index
          ].revProjectUndertakerNumberVoList[setIndex].name
            = 'professional_list-001'
        }
    ...
    addUnitMsg () {
      for (var key in this.projectForm.revProjectUndertakerParamList) {
        if (
          this.projectForm.revProjectUndertakerParamList[key].companyName === ''
        ) {
          let index = key
          document.querySelector(
            `#unitFormStyle${index} .el-autocomplete`
        ...
        let index0 = key
        if (
          document.querySelector(`#unitFormStyle${index0} .el-autocomplete`)
            .style.borderRadius === '4px'
        ) {
          this.isCheck = false
        }
    ...
    addMajorMsg (index, dex) {
      ...
      this.projectForm.revProjectUndertakerParamList[
        index
      ].revProjectUndertakerNumberVoList.push({
        name: '', // 专业名称id
        zhname: '', // 专业名称
        peopleNumber: '' // 人数
      })
    ...
    removeMajorMsg (index, dex) {
      this.projectForm.revProjectUndertakerParamList[
        index
      ].revProjectUndertakerNumberVoList.splice(dex + 1, 1)
    },
    ...
    async handleSelect (item) {
      ...
      if (isRepeat(companyNameList) === true) {
        ...
        this.projectForm.revProjectUndertakerParamList[index].preparatUnitId
          = '' // 编制单位ID
        // this.projectForm.revProjectUndertakerParamList[index].companyName= '' // 编制单位名称
        this.projectForm.revProjectUndertakerParamList[index].contractAmount
          = '' // 合同金额
        this.projectForm.revProjectUndertakerParamList[index].contractTitle
          = '' // 合同名称
        this.projectForm.revProjectUndertakerParamList[index].contractScope
          = '' // 合同范围
        this.projectForm.revProjectUndertakerParamList[index].projectLeader
          = '' // 项目负责人职务
        this.projectForm.revProjectUndertakerParamList[
          index
        ].projectLeaderName = '' // 项目负责人姓名
        ...
        this.projectForm.revProjectUndertakerParamList[index].unitNatureName
          = '' // 单位性质
        this.projectForm.revProjectUndertakerParamList[index].totalPeople = '' // 总人数
        this.projectForm.revProjectUndertakerParamList[
          index
        ].planningLevelName = '' // 城乡规划资质等级
        this.projectForm.revProjectUndertakerParamList[index].mappingLevelName
          = '' // 测绘资质
        this.projectForm.revProjectUndertakerParamList[
          index
        ].directoryLevelName = '' // 推荐名录等级
      ...
      this.projectForm.revProjectUndertakerParamList[index].preparatUnitId
        = item.id
      this.projectForm.revProjectUndertakerParamList[index].companyName
        = item.companyName
      this.projectForm.revProjectUndertakerParamList[index].unitNatureName
        = item.dictTranslate.unitNatureName
      this.projectForm.revProjectUndertakerParamList[index].totalPeople
        = item.totalPeople
      this.projectForm.revProjectUndertakerParamList[index].planningLevelName
        = item.dictTranslate.planningLevelName
      this.projectForm.revProjectUndertakerParamList[index].mappingLevelName
        = item.mappingLevelName
      this.projectForm.revProjectUndertakerParamList[index].directoryLevelName
        = item.dictTranslate.directoryLevelName
      document.querySelector(
        `#unitFormStyle${index} .el-autocomplete`
      ).style.border = ''
      document.querySelector(
      ...
      this.$get(
        ...
        if (res.code === 0) {
          for (var key in res.pager.results) {
            ...
            res.pager.results[key]['value']
              = res.pager.results[key].companyName
    ...
    getBlurUnitMsg (index) {
      setTimeout(() => {
        let nowCompanyName = this.projectForm.revProjectUndertakerParamList[
          index
        ].companyName
        ...
        this.$get(
          `/epf-result-review/preparatUnit/searchByName?companyName=${nowCompanyName}`
        ).then(res => {
          if (res.code === 0) {
            ...
            if (nowCompanyNameList.includes(nowCompanyName) === true) {
              ...
              this.projectForm.revProjectUndertakerParamList[
                index
              ].preparatUnitId = res.pager.results[0].id
              this.projectForm.revProjectUndertakerParamList[
                index
              ].unitNatureName
                = res.pager.results[0].dictTranslate.unitNatureName
              this.projectForm.revProjectUndertakerParamList[
                index
              ].totalPeople = res.pager.results[0].totalPeople
              this.projectForm.revProjectUndertakerParamList[
                index
              ].planningLevelName
                = res.pager.results[0].dictTranslate.planningLevelName
              this.projectForm.revProjectUndertakerParamList[
                index
              ].mappingLevelName = res.pager.results[0].mappingLevelName
              this.projectForm.revProjectUndertakerParamList[
                index
              ].directoryLevelName
                = res.pager.results[0].dictTranslate.directoryLevelName
              document.querySelector(
                `#unitFormStyle${index} .el-autocomplete`
              ).style.border = ''
              document.querySelector(
              ...
              this.projectForm.revProjectUndertakerParamList[index].joinNumber
                = '' // 直接参与人数
              this.projectForm.revProjectUndertakerParamList[
                index
              ].joinRegisterNumber = '' // 直接参与注册规划师人数
    ...
    getCDInputData () {
      for (var key in this.projectForm.revProjectUndertakerParamList) {
        this.projectForm.revProjectUndertakerParamList[key].contractAmount = this.projectForm.revProjectUndertakerParamList[key].contractAmount.replace(/^\D*(\d*(?:\.\d{0,12})?).*$/g, '$1')
        this.projectForm.revProjectUndertakerParamList[key].joinNumber = this.projectForm.revProjectUndertakerParamList[key].joinNumber.replace(/[^\d]/g, '')
        this.projectForm.revProjectUndertakerParamList[key].joinRegisterNumber = this.projectForm.revProjectUndertakerParamList[key].joinRegisterNumber.replace(/[^\d]/g, '')
        for (var index in this.projectForm.revProjectUndertakerParamList[key].revProjectUndertakerNumberVoList) {
          this.projectForm.revProjectUndertakerParamList[key].revProjectUndertakerNumberVoList[index].peopleNumber = this.projectForm.revProjectUndertakerParamList[key].revProjectUndertakerNumberVoList[index].peopleNumber.replace(/[^\d]/g, '')
        }
      }
    }
  },
  ...
  watch: {
    'projectForm.biddingFlag': {
      ...
      handler (val) {
        if (val === 1) {
          if (this.projectForm.revProjectUndertakerParamList.length === 0) {
            // if (this.projectForm.hasOwnProperty('revProjectUndertakerParamList') === false) {
            this.projectForm['revProjectUndertakerParamList'] = [
              {
                preparatUnitId: '', // 编制单位ID
                companyName: '', // 编制单位名称
