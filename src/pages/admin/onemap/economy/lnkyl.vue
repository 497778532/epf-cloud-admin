    ...
    <div class="page-body-nature"
      ...
      <div class="left-top--nature">
        <div class="left-top-button-other-nature">
          <div class="left-top-button-other-nature-title">历年客运量</div>
        </div>
      </div>
      <div class="page-list-nature EPF-table">
        ...
        <div v-if="cjActiveName === 'Table'">
          <el-table :data="listData.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
            ...
            <el-table-column prop="ZJ_WR"
                             label="总计_万人"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('总计_万人')">
              <template slot-scope="scope">
                <span v-if="scope.row.ZJ_WR===''">- -</span>
                <span v-else>{{scope.row.ZJ_WR}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="GL_WR"
                             label="公路_万人"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('公路_万人')">
              <template slot-scope="scope">
                <span v-if="scope.row.GL_WR===''">- -</span>
                <span v-else>{{scope.row.GL_WR}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="SY_WR"
                             label="水运_万人"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('水运_万人')">
              <template slot-scope="scope">
                <span v-if="scope.row.SY_WR===''">- -</span>
                <span v-else>{{scope.row.SY_WR}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="TL_WR"
                             label="铁路_万人"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('铁路_万人')">
              <template slot-scope="scope">
                <span v-if="scope.row.TL_WR===''">- -</span>
                <span v-else>{{scope.row.TL_WR}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="MH_WR"
                             label="民航_万人"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('民航_万人')">
              <template slot-scope="scope">
                <span v-if="scope.row.MH_WR===''">- -</span>
                <span v-else>{{scope.row.MH_WR}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportList () {
      require.ensure([], () => {
        ...
        const tHeader = ['年份', '总计_万人', '公路_万人', '水运_万人', '铁路_万人', '民航_万人', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['NF', 'ZJ_WR', 'GL_WR', 'SY_WR', 'TL_WR', 'MH_WR', 'RKSJ', 'GXSJ', 'PCH', 'ZLSJ', 'ZLBS']
        // 上面的index、nickName、name是tableData里对象的属性
        const list = this.listData // 把data里的tableData存到list
        const data = this.formatJson(filterVal, list)
        export_json_to_excel(tHeader, data, '历年客运量')
      })
    },
    ...
    async getEconomyData (yaerList) {
      ...
      for (let i = 0; i < yaerList.length; i++) {
        let res = await this.$post(`/epf-third-part/oneBatch/LNKYL?NF=${yaerList[i]}`)
        if (res.code === 0) {
      ...
      for (var key in chartData) {
        data0.push(Number(chartData[key].ZJ_WR).toFixed(2))//	总计_万人

        data1.push(Number(chartData[key].GL_WR).toFixed(2))// 公路_万人
        data2.push(Number(chartData[key].SY_WR).toFixed(2))// 水运_万人
        data3.push(Number(chartData[key].TL_WR).toFixed(2))// 铁路_万人
        data4.push(Number(chartData[key].MH_WR).toFixed(2))// 民航_万人
  ...
  created () {
    this.checkList = ['年份', '总计_万人', '公路_万人', '水运_万人', '铁路_万人', '民航_万人', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
    this.checkListIng = ['年份', '总计_万人', '公路_万人', '水运_万人', '铁路_万人', '民航_万人', '入库时间', '更新时间']
