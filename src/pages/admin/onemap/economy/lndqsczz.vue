    ...
    <div class="page-body-nature"
      ...
      <div class="left-top--nature">
        <div class="left-top-button-other-nature">
          <div class="left-top-button-other-nature-title">历年地区生产总值</div>
        </div>
      </div>
      <div class="page-list-nature EPF-table">
        ...
        <div v-if="cjActiveName === 'Table'">
          <el-table :data="listData.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
            ...
            <el-table-column prop="DQSCZZ_YY"
                             label="地区生产总值_亿元"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('地区生产总值_亿元')">
              <template slot-scope="scope">
                <span v-if="scope.row.DQSCZZ_YY===''">- -</span>
                <span v-else>{{scope.row.DQSCZZ_YY}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="RJDQSCZZ_Y"
                             label="人均地区生产总值_元"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('人均地区生产总值_元')">
              <template slot-scope="scope">
                <span v-if="scope.row.RJDQSCZZ_Y===''">- -</span>
                <span v-else>{{scope.row.RJDQSCZZ_Y}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="DQSCZZ_DYCY_YY"
                             label="地区生产总值第一产业_亿元"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('地区生产总值第一产业_亿元')">
              <template slot-scope="scope">
                <span v-if="scope.row.DQSCZZ_DYCY_YY===''">- -</span>
                <span v-else>{{scope.row.DQSCZZ_DYCY_YY}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="DQSCZZDECY_YY"
                             label="地区生产总值第二产业_亿元"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('地区生产总值第二产业_亿元')">
              <template slot-scope="scope">
                <span v-if="scope.row.DQSCZZDECY_YY===''">- -</span>
                <span v-else>{{scope.row.DQSCZZDECY_YY}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="DQSCZZDSCY_YY"
                             label="地区生产总值第三产业_亿元"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('地区生产总值第三产业_亿元')">
              <template slot-scope="scope">
                <span v-if="scope.row.DQSCZZDSCY_YY===''">- -</span>
                <span v-else>{{scope.row.DQSCZZDSCY_YY}}</span>
              </template>
            </el-table-column>
  ...
  methods: {
    ...
    exportList () {
      require.ensure([], () => {
        ...
        const tHeader = ['年份', '地区生产总值_亿元', '人均地区生产总值_元', '地区生产总值第一产业_亿元', '地区生产总值第二产业_亿元', '地区生产总值第三产业_亿元', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['TJNF', 'DQSCZZ_YY', 'RJDQSCZZ_Y', 'DQSCZZ_DYCY_YY', 'DQSCZZDECY_YY', 'DQSCZZDSCY_YY', 'RKSJ', 'GXSJ', 'PCH', 'ZLSJ', 'ZLBS']
        // 上面的index、nickName、name是tableData里对象的属性
        const list = this.listData // 把data里的tableData存到list
        const data = this.formatJson(filterVal, list)
        export_json_to_excel(tHeader, data, '历年地区生产总值')
      })
    },
    ...
    async getEconomyData () {
      ...
      let res = await this.$post('/epf-third-part/threebatch/LNDQSCZZ', params)
      if (res.code === 0) {
        for (let key in res.result) {
          res.result[key]['RankYear'] = parseInt(res.result[key].TJNF)
          if (res.result[key].TJNF.length > 4) {
        ...
        // this.checkList = ['年份', '地区生产总值_亿元', '人均地区生产总值_元', '地区生产总值第一产业_亿元', '地区生产总值第二产业_亿元', '地区生产总值第三产业_亿元', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
        // this.checkListIng = ['年份', '地区生产总值_亿元', '人均地区生产总值_元', '地区生产总值第一产业_亿元', '地区生产总值第二产业_亿元', '地区生产总值第三产业_亿元', '入库时间', '更新时间']
        this.xzhouData = [] // x轴数据
        ...
        for (var key in chartData) {
          data0.push(Number(chartData[key].DQSCZZ_YY).toFixed(2))//	地区生产总值_亿元
          data1.push(Number(chartData[key].RJDQSCZZ_Y).toFixed(2))// 人均地区生产总值_元

          data2.push(Number(chartData[key].DQSCZZ_DYCY_YY).toFixed(2))// 地区生产总值第一产业_亿元
          data3.push(Number(chartData[key].DQSCZZDECY_YY).toFixed(2))// 地区生产总值第二产业_亿元
          data4.push(Number(chartData[key].DQSCZZDSCY_YY).toFixed(2))// 地区生产总值第三产业_亿元
        ...
        if (this.xzhouData.length === 0) {
          ...
          this.legendDataForBarT1 = ['地区生产总值']
          this.legendDataForLineT1 = ['人均地区生产总值']
          this.legendDataForBarLineT1 = ['地区生产总值', { name: '人均地区生产总值', icon: 'line' }]

          this.legendDataForLine1 = ['地区生产总值第一产业', '地区生产总值第二产业', '地区生产总值第三产业']
        ...
        this.seriesAllDataForLine1.push(data2, data3, data4)
