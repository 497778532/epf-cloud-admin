    ...
    <div class="page-body-nature"
      ...
      <div class="left-top--nature">
        <div class="left-top-button-other-nature">
          <div class="left-top-button-other-nature-title">历年旅客周转量</div>
        </div>
      </div>
      <div class="page-list-nature EPF-table">
        ...
        <div v-if="cjActiveName === 'Table'">
          <el-table :data="listData.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
            ...
            <el-table-column prop="ZJ_YRGL"
                             label="总计_亿人公里"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('总计_亿人公里')">
              <template slot-scope="scope">
                <span v-if="scope.row.ZJ_YRGL===''">- -</span>
                <span v-else>{{scope.row.ZJ_YRGL}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="GL_YRGL"
                             label="公路_亿人公里"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('公路_亿人公里')">
              <template slot-scope="scope">
                <span v-if="scope.row.GL_YRGL===''">- -</span>
                <span v-else>{{scope.row.GL_YRGL}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="SY_YRGL"
                             label="水运_亿人公里"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('水运_亿人公里')">
              <template slot-scope="scope">
                <span v-if="scope.row.SY_YRGL===''">- -</span>
                <span v-else>{{scope.row.SY_YRGL}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="TL_YRGL"
                             label="铁路_亿人公里"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('铁路_亿人公里')">
              <template slot-scope="scope">
                <span v-if="scope.row.TL_YRGL===''">- -</span>
                <span v-else>{{scope.row.TL_YRGL}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="MH_YRGL"
                             label="民航_亿人公里"
                             min-width="100"
                             align="center"
                             v-if="checkListIng.includes('民航_亿人公里')">
              <template slot-scope="scope">
                <span v-if="scope.row.MH_YRGL===''">- -</span>
                <span v-else>{{scope.row.MH_YRGL}}</span>
              </template>
            </el-table-column>
  ...
  methods: {
    ...
    exportList () {
      require.ensure([], () => {
        ...
        const tHeader = ['年份', '总计_亿人公里', '公路_亿人公里', '水运_亿人公里', '铁路_亿人公里', '民航_亿人公里', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['NF', 'ZJ_YRGL', 'GL_YRGL', 'SY_YRGL', 'TL_YRGL', 'MH_YRGL', 'RKSJ', 'GXSJ', 'PCH', 'ZLSJ', 'ZLBS']
        // 上面的index、nickName、name是tableData里对象的属性
        const list = this.listData // 把data里的tableData存到list
        const data = this.formatJson(filterVal, list)
        export_json_to_excel(tHeader, data, '历年旅客周转量')
      })
    },
    ...
    async getEconomyData () {
      ...
      let res = await this.$post('/epf-third-part/threebatch/LNLKZZL', params)
      if (res.code === 0) {
        ...
        // this.checkList = ['年份', '总计_亿人公里', '公路_亿人公里', '水运_亿人公里', '铁路_亿人公里', '民航_亿人公里', '入库时间', '更新时间', '批次号', '增量时间', '增量标识']
        // this.checkListIng = ['年份', '总计_亿人公里', '公路_亿人公里', '水运_亿人公里', '铁路_亿人公里', '民航_亿人公里', '入库时间', '更新时间']
        this.xzhouData = [] // x轴数据
        ...
        for (var key in chartData) {
          data0.push(Number(chartData[key].ZJ_YRGL).toFixed(2))//	总计_亿人公里

          data1.push(Number(chartData[key].GL_YRGL).toFixed(2))// 公路_亿人公里
          data2.push(Number(chartData[key].SY_YRGL).toFixed(2))// 水运_亿人公里
          data3.push(Number(chartData[key].TL_YRGL).toFixed(2))// 铁路_亿人公里
          data4.push(Number(chartData[key].MH_YRGL).toFixed(2))// 民航_亿人公里
        ...
        if (this.xzhouData.length === 0) {
          ...
          this.legendDataForLine1 = ['公路', '水运', '铁路', '民航']
