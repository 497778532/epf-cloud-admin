      ...
      <el-form label-width="100px">
        ...
        <div class="tabContent"
          ...
          <el-row style="margin-top:10px;"
            ...
            <el-col :span="5"
                    style="display:flex;justify-content:center;">
              <el-input @input="item.colorMin = item.colorMin.replace(/[^\d]/g, '')"
                        :maxlength="6"
                        v-model="item.colorMin"
                        size="small"
                        style="width:85%;"></el-input>
            </el-col>
            <el-col :span="5"
                    style="display:flex;justify-content:center;">
              <el-input @input="item.colorMax = item.colorMax.replace(/[^\d]/g, '')"
                        :maxlength="6"
                        v-model="item.colorMax"
                        size="small"
                        style="width:85%;"></el-input>
            </el-col>
            <el-col :span="10"
...
export default {
  ...
  methods: {
    ...
    getMapData () {
      ...
      let cantonList = this.jsonData.cantonList
      for (let key in cantonList) {
        this.jsonData.allData.push({
          name: cantonList[key].rowName,
          value: cantonList[key].rowMessage
        })
      }
      // console.log("区域数据", this.jsonData.allData);
      this.jsonData.mapColorData = []
      let colorTableList = this.jsonData.colorTableList
      for (let key in colorTableList) {
        this.jsonData.mapColorData.push({
          start: colorTableList[key].colorMin - 0 || 0,
          end: colorTableList[key].colorMax - 0,
          color: colorTableList[key].colorSelect,
          label: colorTableList[key].colorMessage
        })
      }
    ...
    async loadMap (mapCode, name) {
      ...
      if (data) {
        ...
        // 组装底图数据
        var option = commonecharts.makeColor(this.jsonData, name, centerId)
    ...
    // 分级数量
    getNumberData () {
      var num = this.jsonData.number
      var color = ''
      this.jsonData.colorTableList = []
      for (let i = 0; i < num; i++) {
        color = this.colorList[i].colorSelect
        this.jsonData.colorTableList.push({
          colorSelect: color,
          colorMin: '',
          colorMax: '',
          colorMessage: ''
