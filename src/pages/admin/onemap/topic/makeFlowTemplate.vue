  ...
  <div id="Template"
    ...
    <div class="templateBox">
      ...
      <el-form :model="dataForm"
          ...
          <el-tab-pane label="添加数据"
            ...
            <div class="tabContent">
              ...
              <el-row style="margin-top:10px;"
                      v-for="(item,index) in jsonData.SEList"
                      :key="index">
                <el-col :span="8"
                  ...
                  <el-select size="mini"
                             ...
                             placeholder="选择地区"
                             style="width:100%;">
                    <el-option v-for="(x,i) in cantonList"
                               :key="i"
                               :label="x.zhCn"
                               :value="x.zhCn"></el-option>
                  </el-select>
                </el-col>
                <el-col :span="8"
                        style="display:flex;justify-content:center;">
        ...
        <el-form-item label="流向样式："
                      prop="flowStyle"
                      style="margin-top:10px;">
          <el-checkbox v-model="jsonData.checked">重复播放动画</el-checkbox>
        </el-form-item>

        <el-form-item label="起点："
                      id="startSelect">
          <el-select size="small"
                     v-model="jsonData.flowStart"
                     style="width:100%;"
                     ref="startSelect"
                     @change="startChangeSelection">
            <el-option v-for="item in startSelectImg"
                       :key="item.id"
                       :label="item.src"
                       :value="item.id">
              <img class="selectImage1"
                   :src="item.src" />
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="线型："
                      id="startSelect">
          <el-select size="small"
                     v-model="jsonData.flowLine"
                     style="width:100%;"
                     ref="lineSelect"
                     @change="lineChangeSelection">
            <el-option v-for="item in lineSelectImg"
                       :key="item.id"
                       :label="item.src"
                       :value="item.id">
              <img class="selectImage2"
                   :src="item.src" />
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="终点："
                      id="startSelect">
          <el-select size="small"
                     v-model="jsonData.flowEnd"
                     style="width:100%;"
                     ref="endSelect"
                     @change="endChangeSelection">
            <el-option v-for="item in endSelectImg"
                       :key="item.id"
                       :label="item.src"
                       :value="item.id">
              <img class="selectImage1"
    ...
    <div style="display:flex;flex-direction: column;align-items: flex-end;flex:1;height:100%;">
      ...
      <div id="mapBox"
           ref="chinaChart"></div>
      <!-- <img src="../../../../assets/images/flow.png" alt width="100%" height="95%" /> -->
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      startSelectImg: [
        {
          src: require('@/assets/images/startCircle0.png'),
          id: '0'
        },
        {
          src: require('@/assets/images/startCircle1.png'),
          id: '1'
        },
        {
          src: require('@/assets/images/startCircle2.png'),
          id: '2'
        },
        {
          src: require('@/assets/images/startCircle3.png'),
          id: '3'
        }
      ],
      endSelectImg: [
        {
          src: require('@/assets/images/endCircle0.png'),
          id: '0'
        },
        {
          src: require('@/assets/images/endCircle1.png'),
          id: '1'
        },
        {
          src: require('@/assets/images/endCircle2.png'),
          id: '2'
        },
        {
          src: require('@/assets/images/endCircle3.png'),
          id: '3'
        }
      ],
      lineSelectImg: [
        ...
        {
          src: require('@/assets/images/line4.png'),
          id: '1'
        },
        {
          src: require('@/assets/images/line5.png'),
          id: '2'
        }
      ],
      colorSelectImg: [
      ...
      startSelectPath: require('@/assets/images/endCircle0.png'),
      endSelectPath: require('@/assets/images/endCircle0.png'),
      lineSelectPath: require('@/assets/images/line2.png'),
      colorSelectPath: require('@/assets/images/color0.png')
    }
  },
  methods: {
    inputCheck (val) {
      for (var key in this.jsonData.SEList) {
        this.jsonData.SEList[key].message = this.jsonData.SEList[key].message.replace(/[^\d]/g, '')
      }
      if (val.length >= 6) {
        this.$message.error('只能输入不大于5位数的数字!')
      }
    },
    startChangeSelection (val) {
      setTimeout(() => {
        this.startSelectPath = this.$refs.startSelect.selectedLabel
        this.$refs.startSelect.$el.children[0].children[0].setAttribute(
          'style',
          'background:url('
          + this.startSelectPath
          + ') no-repeat 20px center/24px 24px;color:transparent;'
        )
        console.log(val)
      }, 100)
    },
    endChangeSelection (val) {
      setTimeout(() => {
        this.endSelectPath = this.$refs.endSelect.selectedLabel
        this.$refs.endSelect.$el.children[0].children[0].setAttribute(
          'style',
          'background:url('
          + this.endSelectPath
          + ') no-repeat 20px center/24px 24px;color:transparent;'
    ...
    lineChangeSelection (val) {
      setTimeout(() => {
        this.lineSelectPath = this.$refs.lineSelect.selectedLabel
        this.$refs.lineSelect.$el.children[0].children[0].setAttribute(
          'style',
          'background:url('
          + this.lineSelectPath
          + ') no-repeat 20px center/100px 24px;color:transparent;'
        )
        console.log(val)
      }, 100)
    },
    ...
    provinceSelect (name) {
      ...
      this.$get('/epf-admin/regionals/getAreaByCodeOrParentId', {
        ...
        this.cantonList = res.dictionariesList
        ...
        this.jsonData.SEList = provinceNameList
      })
    },
    // 字典-市选择
    citySelect (name) {
    ...
    async loadMap (mapCode, name) {
      ...
      if (data) {
        that.chart = echarts.init(that.$refs.chinaChart)
        ...
        // 组装底图数据
        var option = commonecharts.makeFlow(that.jsonData, name, mapCenter)
...
<style>
#startSelect .el-input__inner::placeholder {
  color: transparent;
}
</style>
