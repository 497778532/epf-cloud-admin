<template>
  <div v-if="nowcollideDialog"
       id="nowcollide"
       v-drag
       class="EPF-table">
    <div class="data_title_different drag-title">
      <span>规划与现状冲突检测结果</span>
      <i class="el-icon-close"
         @click="handleClickClose"></i>
    </div>
    <div class="data_content_different">
      <el-divider></el-divider>
      <el-row class="screenChange">
        <el-col :span="12"
                class="screenChange"><span>规划与现状均为建设用地：</span><span>9374m²</span>
        </el-col>
        <el-col :span="12"
                class="screenChange"><span>规划建设用地，现状建设用地：</span><span>3726m²</span>
        </el-col>
      </el-row>
      <el-row class="screenChange">
        <el-col :span="12"
                class="screenChange"><span>规划与现状均为非建设用地：</span><span>12847m²</span>
        </el-col>
        <el-col :span="12"
                class="screenChange"><span>规划建设用地，现状非建设用地：</span><span>9857m²</span>
        </el-col>
      </el-row>
      <el-row style="height: 40px; display: flex;justify-content: flex-start;align-items: center;">
        <el-col :span="12">
          <div style="width:100%;height:100%;"></div>
        </el-col>
        <el-col :span="12"
                style="height: 40px; display: flex;justify-content: flex-end;align-items: center;">
          <div class="content_unitSelect">
            <div style="width:100px;">面积单位：</div>
      ...
      <el-table :data="dataList"
                ...
                style="width: 100%;min-height:80px;margin-top:16px;"
                border>
        ...
        <el-table-column prop="chongtuType"
                         ...
                         min-width="250"
                         align="center"></el-table-column>
        <el-table-column prop="guihuaType"
                         label="规划地类名称"
                         min-width="250"
                         align="center"></el-table-column>
        <el-table-column prop="xianzhuangName"
                         label="现状地类名称"
                         min-width="150"
                         align="center"></el-table-column>
      ...
      <exportChart :chartData="dataList"
                   @close="isChartShow = false"
                   :isChartShow="isChartShow"
                   :chartName="titleName"
                   :chartUnitName="unitData"
                   ref="differentChart"></exportChart>
    </div>
  </div>
</template>

<script>
import exportChart from './export-chart-own'
export default {
  props: {
    nowcollideDialog: Boolean,
    tableData: Array,
  ...
  data () {
    return {
      ...
      totalArea: 0,
      excelFileName: '规划与现状冲突检测分析结果.xls',
      excelHeader: {},
      spanArr: [],
  ...
  methods: {
    ...
    showExport (type) {
      if (type === 'chart') {
        ...
        this.exportRaw('规划与现状冲突检测分析结果.txt', result)
      } else if (type === 'execl') {
      }
    },
    fakeClick (obj) {
