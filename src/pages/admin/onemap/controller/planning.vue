<template>
  <div v-if="planningDialog"
       id="planning"
       v-drag>
    <div class="data_title drag-title">
      <span>规划数据分析结果</span>
    ...
    <div class="data_content">
      <div style="margin-bottom: 5px">
        <el-radio-group v-model="tableDataType"
                        size="small"
                        @change="switchTableData">
          <el-radio-button label="默认"></el-radio-button>
          <el-radio-button label="详细"></el-radio-button>
        </el-radio-group>
        <div>
          <p>图斑总面积：<span v-show="!loading">{{totalArea}}</span></p>
        </div>
      </div>
      ...
      <export-chart :tableData="currentTableData"
                    @close="isChartShow = false"
                    :isChartShow="isChartShow"></export-chart>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    switchTableData () {
      if (this.tableDataType === '默认') {
        this.currentTableData = this.tableData.filter(e => e.type !== 'GHYT2')
      } else if (this.tableDataType === '详细') {
        this.currentTableData = this.tableData.filter(e => e.type !== 'GHYT1')
      }
      this.calcSpanArr()
    },
    calcSpanArr () {
