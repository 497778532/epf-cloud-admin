...
export default {
  ...
  methods: {
    handleClickClose() {
      this.$emit('closeDialog', 'superpositionDialog')
    },
  ...
  watch: {
    superpositionDialog(val) {
      if (val) {
        this.updateTableHeader()
      }
    },
    tableData() {
      if (this.updateTableHeader) {
        this.updateTableHeader()
      }
    }
  },
...
#superposition .button-list {
  text-align: right;
  margin: 8px 0px;
  margin-top: 15px;
}

#superposition .data_title {
  width: 800px;
  height: 45px;
  color: #fff;
  font-size: 18px;
