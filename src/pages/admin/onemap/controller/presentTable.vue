<template>
  <div v-if="isPTableShow" id="m-table" v-drag>
    ...
    <div class="data_content">
      <div style="margin-bottom: 5px">
        <el-radio-group v-model="tableDataYear" size="small" @change="switchTableDataYear">
...
export default {
  ...
  methods: {
    handleClickClose() {
      this.$emit('closeDialog', 'isPTableShow')
    },
    ...
    switchTableDataYear(val) {
      let year = parseInt(val)
      this.currentTableData = this.tableData.filter(e => e.year === year)
      this.singleYearData = this.currentTableData
      this.switchTableData(this.tableDataType)
    },
    switchTableData(val) {
      let data = this.singleYearData
      if (val === '默认') {
    ...
    changeToDefault(data) {
      ...
      this.trendData = this.tableData.filter(e => e.landuseCode.slice(2, 4) === '00')
    },
    changeToSpecific(data) {
      this.currentTableData = data.filter(
        e => e.landuseCode.slice(2, 4) === '--'
      )
      this.trendData = this.tableData.filter(e => e.landuseCode.slice(2, 4) === '--')
    },
    calcSpanArr() {
      this.spanArr = []
      ...
      for (var i = 0; i < len; i++) {
        if (i === 0) {
          ...
          // 判断当前元素与上一个元素是否相同
          if (
            this.currentTableData[i].DK_ID
            === this.currentTableData[i - 1].DK_ID
          ) {
    ...
    updateTableHeader() {
      ...
      // this.tableDataYear = this.multiResult[0].name;
      this.switchTableDataYear(this.tableDataYear)
    },
    showExport(type) {
      if (type === 'chart') {
